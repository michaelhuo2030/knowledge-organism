...
# ---- PROPOSE -> EXECUTE bridge — agenda generates a born-compliant experiment SKELETON ----
def scaffold(law, name=None, move=""):
    """Generate a BORN-COMPLIANT experiment skeleton for an agenda item: law declared, ≥3 seeds,
    held-out/baseline/ruler pre-wired, emit ready, + an anti-fabrication FILLED lock that refuses to
    emit until the body+ruler are written. Automates the discipline scaffolding, not the science."""
    name=name or f"{law.lower()}_scaffold"; kitdir=str(pathlib.Path(__file__).resolve().parent)
    tpl=f'''#!/usr/bin/env python3
"""AUTO-SCAFFOLD for {law} — {move or "agenda-proposed experiment"}
Generated by the organism's active loop. Born compliant. FILL experiment() + the PRE-REGISTERED
RULER, set FILLED=True, then run. Until FILLED it refuses to emit (anti-fabrication lock)."""
import os, sys
sys.path.insert(0, {kitdir!r})
import organism

FILLED = False        # set True ONLY after experiment()+ruler are real (do NOT fake numbers)
LAW = "{law}"
SEEDS = [0, 1, 2]     # >=3 clears the immune gate

# PRE-REGISTERED RULER (write BEFORE running):
#   PASS if : <TODO falsifiable threshold>
#   baseline: <TODO beats-random / anchor>
#   held-out: <TODO disjoint eval split>

def experiment(seed):
    raise NotImplementedError("fill experiment(); return a dict of REAL metrics")

def main():
    if not FILLED:
        print("[scaffold] not filled — refusing to emit. Write experiment()+ruler, set FILLED=True."); return
    res=[experiment(s) for s in SEEDS]
    summary={{}}                                   # TODO aggregate REAL numbers across seeds
    csv="results_{name}.csv"                       # TODO write the real rows
    organism.Kit("{name}", tests=[LAW], could_update=[LAW]).emit(
        verdict="POC", result=summary, note="<TODO honest finding + boundary>",
        csv_path=csv, held_out=True, baseline="<TODO>", seeds=len(SEEDS))

if __name__=="__main__": main()
'''
    dest=pathlib.Path(f"scaffold_{name}.py"); dest.write_text(tpl)
    print(f"[scaffold] wrote {dest} — fill experiment()+ruler, set FILLED=True, then run."); return str(dest)

def propose():
    """Self-driving step: run the agenda, then SCAFFOLD the #1 move into a runnable skeleton."""
    items=agenda(top=3)
    if not items: print("[propose] empty agenda"); return
    top=items[0]; print(f"\n[propose] top move = {top['law']}: {top['move']}\n[propose] scaffolding …")
    return scaffold(top["law"], move=top["move"])
...
if __name__=="__main__":
    ...
    elif a[0]=="propose": propose()
    elif a[0]=="scaffold" and len(a)>1: scaffold(a[1])
